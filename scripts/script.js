...
$('#btn').click(function() {
  ...
  $.getJSON(
    'https://cors-anywhere.herokuapp.com/http://api.openweathermap.org/data/2.5/weather?q=' +
      city +
      '&units=metric&APPID=8e4dfa289871334662836ee6616527d4',
    function(data) {
      $('.temp').empty();
      $('.weather').empty();
      var icon =
  ...
  // geocode api
  $.getJSON(
    'https://cors-anywhere.herokuapp.com/http://open.mapquestapi.com/geocoding/v1/address?key=nQm9XprUAmoXluOdCqAj7UiNOAY3M2ld&location=' +
      city,
    function(response) {
...
$('#mapbtn').click(function() {
  ...
  $.getJSON(
    'https://cors-anywhere.herokuapp.com/http://api.airpollutionapi.com/1.0/aqi?lat=' +
      lat +
      '&lon=' +
      lng +
      '&APPID=s4cebehbqs4dn32tfm1bli2v3i',
    function(res) {
      $('#aqialert').empty();
      $('#aqipara').empty();
      $('#aqiparameter').empty();
